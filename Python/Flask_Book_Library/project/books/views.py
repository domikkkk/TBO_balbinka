...
@books.route("/filter-unsafe/<string:book_name>", methods=["GET"])
def get_book_details(book_name):
    try:
        query = f"SELECT id, name, author, year_published, book_type, status FROM books WHERE name = '{book_name}'"

        result = db.session.execute(db.text(query))
        books_rows = result.fetchall()
        books_list = [dict(row._mapping) for row in books_rows]
